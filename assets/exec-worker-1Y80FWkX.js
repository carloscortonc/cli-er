  ...
  function Ee(e) {
    ...
    e: for (let u = 0; u < n.length; u++) {
      ...
      for (let k = 0; k < P.length; k++) {
        ...
        let V = s[v2].default, _2 = Object.values(s[v2].options || {}).filter((M) => M.kind === "command").flatMap((M) => M.aliases);
...
`), "WARN"), j();
  ...
  function xe(e, n, t) {
    ...
    if (i.length > 0) {
      if (o && ["namespace", "command"].includes(o.kind)) {
        let d = o.aliases && o.aliases.length > 1 ? " ".concat(g.formatMessage("generate-help.aliases", {
          aliases: o.aliases.slice(1).join(", ")
        })) : "";
        l.description = (O = o.description) == null ? void 0 : O.concat(d, `
`), r = o.options;
      } else g.logger.log(`
...
`));
    ...
    }, b = `${g.formatMessage("generate-help.usage")}:  ${t.cliName}`, m = [b, i.slice(0, -1).join(" "), o != null && o.aliases ? o.aliases[0] : i.slice(-1)[0]].filter((d) => d).join(" "), f = a.some((d) => typeof d.index == "number" && d.index < 0);
