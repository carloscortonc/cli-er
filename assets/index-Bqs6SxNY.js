...
`), process.exit(1);
  ...
  function me(e, n) {
    ...
    for (let i2 = t.length; i2 > 0; i2--) for (let o2 of n) {
      let r = G.default.join(...t.slice(0, i2), o2);
...
`;
  ...
  function Fe(e) {
    return e.kind === "namespace" || !e.rawAliases ? [e.key] : e.kind === "command" ? e.rawAliases : e.rawAliases.map((n) => n.startsWith("-") ? (H({
      property: "Option.aliases with dashes",
      description: `Aliases should be specified without dashes (${e.key})`
  ...
  function Ee(e) {
    ...
    e: for (let u = 0; u < n.length; u++) {
      ...
      for (let k = 0; k < P.length; k++) {
        ...
        let V = s2[v].default, _ = Object.values(s2[v].options || {}).filter((M) => M.kind === "command").flatMap((M) => M.aliases);
...
`), "WARN"), j();
  ...
  function xe(e, n, t) {
    ...
    if (i2.length > 0) {
      if (o2 && ["namespace", "command"].includes(o2.kind)) {
        let d = o2.aliases && o2.aliases.length > 1 ? " ".concat(g.formatMessage("generate-help.aliases", {
          aliases: o2.aliases.slice(1).join(", ")
        })) : "";
        l.description = (O = o2.description) == null ? void 0 : O.concat(d, `
`), r = o2.options;
      } else g.logger.log(`
...
`), i2 = [];
    } else t.cliDescription && (l.description = t.cliDescription.concat(`
`));
    let {
      existingKinds: c,
      hasOptions: s2,
    ...
    }, b = `${g.formatMessage("generate-help.usage")}:  ${t.cliName}`, m = [b, i2.slice(0, -1).join(" "), o2 != null && o2.aliases ? o2.aliases[0] : i2.slice(-1)[0]].filter((d) => d).join(" "), f = a.some((d) => typeof d.index == "number" && d.index < 0);
...
function WorkerWrapper(options) {
  return new Worker(
    "/cli-er/assets/exec-worker-1Y80FWkX.js",
    {
      type: "module",
      name: options?.name
    }
